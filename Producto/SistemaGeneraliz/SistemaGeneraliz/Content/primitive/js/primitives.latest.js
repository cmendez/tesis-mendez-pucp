/**
 * @preserve Basic Primitives orgDiagram v1.0.37
 *
...
/*
    ...
    SubAdviser - Child item which is placed at the same level as parent item. It has connection on its top.
*/
	...
	for (index = 0; index < segments.length; index += 1) {
		...
		switch (segment.segmentType) {
			...
			case 2/*primitives.common.SegmentType.QuadraticArc*/:
				...
				break;
			case 3/*primitives.common.SegmentType.CubicArc*/:
				data += " c" + 10 * Math.round(segment.cpX1) + "," + 10 * Math.round(segment.cpY1) + "," + 10 * Math.round(segment.cpX2) + "," + 10 * Math.round(segment.cpY2) + "," + 10 * Math.round(segment.x) + "," + 10 * Math.round(segment.y);
				break;
		}
	}
	data += " e";
...
primitives.orgdiagram.Controller.prototype._mouseStop = function (event) {
...
primitives.orgdiagram.Controller.prototype._redrawConnector = function (hasGraphics, segments, parentTreeItem, parentTreeLevel) {
	...
	if (parentTreeItem.visualChildren.length > 0) {
		...
		if (!hideConnectors) {
			this.transform.transformPoints(parentHorizontalCenter, parentTreeItem.actualPosition.bottom(),
...
primitives.orgdiagram.Controller.prototype._getTreeItemForMousePosition = function (x, y) {
	var	result = null,
		...
		itemIndex,
		currentDistance,
		placeholder = this.graphics.activate("placeholder", 6/*primitives.orgdiagram.Layers.Item*/);
...
primitives.orgdiagram.Controller.prototype._createVisualTree = function (logicalParentItem) {
	...
	for (index = 0, len = itemLogicalChildren.length; index < len; index += 1) {
		...
		if (this._hasVisibleChildren(itemConfig)) {
			...
			this._getFirstLogicalVisibleParent(logicalParentItem).logicalChildren.push(treeItem);
...
primitives.orgdiagram.Controller.prototype._getFirstLogicalVisibleParent = function (logicalParentItem) {
	var result = logicalParentItem;
	if (logicalParentItem.visibility === 4/*primitives.orgdiagram.Visibility.Invisible*/) {
		if (logicalParentItem.logicalParentId != null) {
			result = this._getFirstLogicalVisibleParent(this._treeItems[logicalParentItem.logicalParentId]);
		}
	}
	return result;
};
...
primitives.orgdiagram.Controller.prototype._showCursorNeigbours = function () {
	...
	if (this._cursorTreeItem !== null) {
		...
		logicalParentItem = this._treeItems[this._cursorTreeItem.logicalParentId];
		if (logicalParentItem != null) {
			logicalParentItem = this._getFirstLogicalVisibleParent(logicalParentItem);
			for (index = 0; index < logicalParentItem.logicalChildren.length; index += 1) {
				treeItem = logicalParentItem.logicalChildren[index];
				if (treeItem.visibility === 0/*primitives.orgdiagram.Visibility.Auto*/) {
					treeItem.visibility = 1/*primitives.orgdiagram.Visibility.Normal*/;
				}
			}
